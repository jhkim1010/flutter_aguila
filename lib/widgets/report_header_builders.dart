...
class ReportHeaderBuilders {
  ...
  /// Unit 선택 DropdownButton 빌드 (좁은 화면용)
  static Widget buildUnitDropdown({
    required BuildContext context,
    required String currentUnit,
    required Color reportColor,
    required Function(String) onUnitChanged,
    ...
    // 현재 unit이 유효한지 확인하고, 없으면 기본값으로 설정
    final validUnits = ['vcode', 'day', 'month', 'year'];
    final displayUnit = validUnits.contains(currentUnit) ? currentUnit : 'vcode';
    ...
    return Container(
      ...
      child: DropdownButton<String>(
        ...
        icon: Icon(Icons.arrow_drop_down, color: reportColor, size: 24),
        style: TextStyle(
          color: reportColor,
          fontSize: 14,
  ...
  /// 날짜 범위 선택 버튼 빌드
  static Widget buildDateRangeButton({
    required BuildContext context,
    required DateTime? startDate,
    required DateTime? endDate,
    ...
    String rangeText = 'Seleccionar rango';
    ...
    if (startDate != null && endDate != null) {
      if (unit == 'year' || unit == 'month') {
        rangeText = '${dateFormat.format(startDate)} - ${dateFormat.format(endDate)}';
      } else {
        rangeText = '${dateFormat.format(startDate)} ~ ${dateFormat.format(endDate)}';
      }
    }
  ...
  static Future<void> selectDateRange(
    ...
    if (unit == 'year') {
      ...
      final defaultStartDate = startDate ?? today;
      final defaultEndDate = endDate ?? today;
      ...
      debugPrint('🔍 selectDateRange - 큰 화면 체크: isDesktop=$isDesktop, isIPad=$isIPad, isLandscape=$isLandscape, size=$size, isLargeScreen=$isLargeScreen');
      
      if (isLargeScreen) {
        debugPrint('   → 큰 화면: 두 개의 달력을 나란히 표시');
        // 큰 화면: 두 개의 달력을 나란히 표시
        final result = await _showDualCalendarPicker(
          context: context,
        ...
        if (result != null) {
          final finalStartDate = result.start.isAfter(today) ? today : result.start;
          final finalEndDate = result.end.isAfter(today) ? today : result.end;
        ...
        if (picked != null) {
          final finalStartDate = picked.start.isAfter(today) ? today : picked.start;
          final finalEndDate = picked.end.isAfter(today) ? today : picked.end;
          onDateRangeChanged(finalStartDate, finalEndDate);
        }
      }
    }
  ...
  /// 연도 범위 선택
  static Future<void> _selectYearRange(
    BuildContext context,
    DateTime? startDate,
    DateTime? endDate,
    Color reportColor,
    ...
    final startYear = await selectYearDialog(
      context,
    ...
    if (startYear != null) {
      ...
      final endYear = await selectYearDialog(
        context,
      ...
      if (endYear != null) {
        ...
        onDateRangeChanged(adjustedStartDate, adjustedEndDate);
      }
    }
  }

  /// 월 범위 선택
  static Future<void> _selectMonthRange(
    BuildContext context,
    DateTime? startDate,
    DateTime? endDate,
    Color reportColor,
    ...
    final startMonth = await selectYearMonthDialog(
      context,
      startDate ?? DateTime.now(),
      endDate,
      reportColor,
    ...
    if (startMonth != null) {
      ...
      final endMonth = await selectYearMonthDialog(
        context,
        endDate ?? adjustedStartDate,
        null,
        reportColor,
  ...
  /// 연도 선택 다이얼로그
  static Future<DateTime?> selectYearDialog(
    BuildContext context,
  ...
  /// 연도와 월 선택 다이얼로그
  static Future<DateTime?> selectYearMonthDialog(
    BuildContext context,
    DateTime initialDate,
    DateTime? maxDate,
    Color reportColor,
  ...
  static Future<DateTimeRange?> _showDualCalendarPicker({
    required BuildContext context,
    required DateTime startDate,
    required DateTime endDate,
    required Color reportColor,
